...
const totalPriceWithoutSale = products && products.reduce((sum, item) => sum + ((item.old_price ? item.old_price : item.price) * item.count), 0);
const totalPrice = products && products.reduce((sum, item) => sum + (item.price * item.count), 0);
const totalWeight = products && products.reduce((sum, item) => sum + ((item.weight_unit === 'кг' ? item.weight : item.weight / 1000) * item.count), 0);
